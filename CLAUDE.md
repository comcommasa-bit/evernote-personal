...
- **v28 (2026-03-14)**: UI全面刷新・minSdk修正・全体チェック
  - サイドバー幅 200→165px、テーマボタン下部コーナーへ移動
  - サイドバー全アイテムにアイコン追加（_NavTileに統合）
  - 検索バーの余分グレー枠除去・クリーンデザイン
  - ノートカード: 角丸+影付き、タイトル左・サムネイル右レイアウト
  - 日付: 今日=HH:MM、過去=M/D フォーマット
  - チェックボックスプレビュー: ☑/☐ → [v]/[ ] (ASCII)
  - テーマ選択ダイアログ刷新
  - Android minSdk を 21→23 に修正（local_auth 2.x 要件）
